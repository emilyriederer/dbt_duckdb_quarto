...
query = """
  select 
    ...
    avg(case when order_date = first_order then 1 else 0 end) as pct_order_new_cust 
  from
    orders 
      left join 
    customers
...
display(Markdown("""
The orders table is updated to contain date through {max_date}.
""".format(max_date = res.end_date.max())))
```
